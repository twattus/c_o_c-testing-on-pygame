...
unit_data=[[1000,0,120,300,0,0,64,-1], #0:tower
           [40,1,60,32,1,0,24,2],#1:generic fellow
           [500,0.4,120,100,2,3,36,4], #2:big boi
           [50,2,6,128,1,2,20,3], #3: speedy fellow
           [],
           []
           ] 
            ...
            for f in range(0,len(self.units)):
                ...
                if pygame.Rect.colliderect(self.projectiles[e].rect,self.units[f].rect):
                    ...
                    if self.units[f].hp<=0:
                        self.units[f].x-=10000 #same as with proj, its just safer to do it this way
                    self.projectiles[e].x-=10000 #casts the projectile into the void, because removing items during iteration is severely dangerous
                    break
        self.validate()
        ...
        for e in range(0,len(temp_p1_choices)):
            if temp_p1_choices[e]:
                if p1_points_bar>=unit_data[p1_options[e]][7]:
                    ...
                    p1_options[e]=random.choice([1,1,1,2,3,3])
                    break
        ...
        for e in range(0,len(temp_p2_choices)):
            if temp_p2_choices[e]:
                if p2_points_bar>=unit_data[p2_options[e]][7]:
                    ...
                    p2_options[e]=random.choice([1,1,2,3,3])
                    break
...
while True:
    ...
    if game_over:
        ...
        while len(p1_options)<4:
            p1_options.append(random.choice([1,1,2,3,3]))
        while len(p2_options)<4:
            p2_options.append(random.choice([1,1,2,3,3]))
